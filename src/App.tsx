...
import { MusicalPalette, type MusicalTool } from './components/organisms/MusicalPalette';
...
const EditorApp = () => {
  ...
  // Sync selected tool with dropdowns
  useEffect(() => {
    if (selectedTool) {
      if (selectedTool.type === MusicalElementType.NOTE) {
        setSelectedDuration(selectedTool.duration);
      } else if (selectedTool.type === MusicalElementType.REST) {
        const restValue = `${selectedTool.duration}-rest`;
        setSelectedRest(restValue);
      }
    }
  }, [selectedTool]);
  ...
  const handleAddRest = (staffId: string, barIndex: number, beatIndex: number, duration: NoteDuration) => {
    console.log('Adding rest:', { staffId, barIndex, beatIndex, duration });
        ...
        if (staff.id === staffId) {
          const updatedBars = staff.bars.map((bar, bIdx) => {
            if (bIdx === barIndex) {
              const updatedBeats = bar.beats.map((beat, btIdx) => {
                if (btIdx === beatIndex) {
                  // Add rest to this beat - rests have different structure than notes
                  const newRest = {
                    id: `rest-${Date.now()}-${Math.random()}`,
                    type: 'rest' as const, // Important: mark as rest type
                    duration,
                    beatIndex,
                    subdivisionOffset: 0,
                    visualOffsetX: 0,
                    visualOffsetY: 0,
                    // Rests don't have pitch/octave properties
                  };
                  return {
                    ...beat,
                    notes: [...beat.notes, newRest] // Rests are stored in same array as notes
                  };
                }
                return beat;
              });
  ...
  const handleToolSelect = (tool: MusicalTool) => {
    ...
    if (tool.type === MusicalElementType.NOTE) {
      setSelectedDuration(tool.duration);
    } else if (tool.type === MusicalElementType.REST) {
      const restValue = `${tool.duration}-rest`;
      setSelectedRest(restValue);
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ margin: '0 0 16px 0', color: '#f0f0f0', fontSize: '16px' }}>
              Staff Editor - Mode: {mode} - Selected: {selectedStaffId} - Tool: {selectedTool?.name || 'None'}
            </h2>
            
            {staffs.map(staff => {
              // Calculate canvas width based on number of bars
              const barStartX = 130;
              const finalBarX = barStartX + (staff.bars.length * 200) + 60;
              ...
              return (
                <MusicStaffCanvas
                  ...
                  darkMode={true}
                  selectedDuration={selectedTool?.type === MusicalElementType.NOTE ? selectedTool.duration : selectedDuration}
