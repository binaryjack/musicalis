const fs = require('fs');
fs.writeFileSync('src/tests/units/music-helpers.test.ts', \import { reconstructBarNotes, parseTimeSignature, validateMeasureMatrix } from '../../shared/utils/music-helpers';
...
describe('music-helpers', () => {
  ...
  describe('validateMeasureMatrix', () => {
    it('should allow placing a note in an empty measure', () => {
      ...
      expect(validateMeasureMatrix(emptyBar, { duration: 'quarter', beatIndex: 0, subdivisionOffset: 0 }, timeSig44)).toBe(true);
    });
    ...
    it('should flag overlapping notes as invalid if beat is occupied', () => {
      ...
      expect(validateMeasureMatrix(occupiedBar, { duration: 'quarter', beatIndex: 1, subdivisionOffset: 0 }, timeSig44)).toBe(false);
    });

    it('should flag a note that exceeds the measure boundaries', () => {
      const emptyBar: Bar = { index: 0, beats: [ { index: 0, notes: [] } ] };
      
      expect(validateMeasureMatrix(emptyBar, { duration: 'half', beatIndex: 3, subdivisionOffset: 0.5 }, timeSig44)).toBe(false);
    });
    ...
    it('should ignore pure rests when validating space (volatile rests)', () => {
      ...
      expect(validateMeasureMatrix(restBar, { duration: 'quarter', beatIndex: 2, subdivisionOffset: 0 }, timeSig44)).toBe(true);
    });
  });
